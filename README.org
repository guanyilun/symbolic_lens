...
│   └── l12_sum.py          # Layer 5: CMB-specific compiler (W² summation → numerical)
│
│ # Tests
...
** Per-kernel: symbolic vs brute-force 3j summation
...
1. The symbolic compiler pipeline (=sym_utils/l12_sum.py=) at
   ~lmax=10, rlmin=1, rlmax=8~.
2. A direct double-sum $\sum_{\ell_1, \ell_2}$ using =sympy.physics.wigner.wigner_3j=
   (exact rational 3j symbols).

Both paths produce the same values to ~$10^{-10}$ relative difference
across every $L$. The ratio plot =comp_sym_vs_brute_ratio.png= is
visually indistinguishable from unity for all 8 kernels:

#+begin_center
[[file:comp_sym_vs_brute_ratio.png]]
#+end_center

This is the strongest possible per-kernel test — brute-force enumeration
with exact 3j arithmetic has no free parameters or approximations.

** End-to-end: symbolic vs hand-coded (pytempura-validated) estimators

=check_full_norm.py= combines the 8 symbolic kernels via the
Toshiya-Namikawa recipe (see =tempura.jl=) to form the quadratic-estimator
normalizations ~qtt, qte, qtb, qee, qbb, qeb~, and compares against the
hand-coded implementations in =norm_lens.py= (which has been validated
against =pytempura= at $\text{rtol} = 10^{-9}$ — see =comp_tempura.py=).

At ~lmax=60, rlmin=2, rlmax=50~ with representative CMB-like spectra,
all 6 estimators agree with the hand-coded reference to roughly
machine precision ($\sim 10^{-14}$):

| Estimator | Max $|\text{hand} - \text{sym}| / |\text{sym}|$ over $L \in [10, 60]$ |
|-----------+---------------------------------------------------------------------|
| qtt       | ~9 × 10^{-15}~                                                      |
| qte       | ~1.3 × 10^{-14}~                                                    |
| qtb       | ~2.1 × 10^{-15}~                                                    |
...
The relative-difference plot =comp_full_norm_reldiff.png= shows the
agreement is flat across $L$:

#+begin_center
[[file:comp_full_norm_reldiff.png]]
#+end_center

And the absolute curves =comp_full_norm.png= overplot indistinguishably:

#+begin_center
[[file:comp_full_norm.png]]
#+end_center

Taken together: the symbolic compiler reproduces the per-triangle Wigner
3j summation (brute-force ground truth) and, when composed into full
estimator normalizations, matches the pytempura-validated hand-coded
implementation at floating-point precision. Any future physics extension
that goes through the same compiler inherits this level of trust.
