...
1. *Grammar enumerator* — candidates are currently hand-picked.  BFS
   over the DSL the compiler already accepts (see "Search-space scope"
   below), canonicalized by plain structural dedup to start.  ~50-100
   lines of enumerator + a few hundred of scoring harness.
2. *E-graph collapse* — if structural dedup proves insufficient,
   canonicalize via equality saturation (see section below).  Defer
   until enumeration sizes force the issue.
...
*First-enumerator config (already decided).*
- m-triples with $|m_*| \le 3$, $\sum m = 0$  (~25 triples).
- Per-leg filter: product of $\le 2$ factors from
  $\{1, a(\ell, s)\text{ for } s \in [-3,3], C\text{-spectra},
  \sqrt{\ell(\ell+1)}\}$.
- Coefficient in $\{1, -1, i, -i\}$ and $P \in \{0, 1\}$.
- Canonicalization: plain structural dedup (e-graph deferred).

Deferred questions at implementation time:
- Whether to auto-restrict spectrum atoms to the input-field type
  (T-leg gets only $C^T$-like spectra) vs allow mismatches that
  score badly but might surface unexpected couplings.
...
See also =memory/search_space.md= for the full scope analysis.
