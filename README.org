...
** Estimator compiler milestone 1 — pixell-backed SHT emitter

Status: milestone 0 (analyzer-only) complete and validated structurally
against falafel; see [[Estimator Compiler][Estimator Compiler]] section above.
The next milestone wires the =EstimatorTerm= recipe list to an actual
SHT backend:

1. *γ-stripping pass.* Walk each EstimatorTerm, remove
   $\sqrt{2\ell_1+1}$ from =X_filter=, $\sqrt{2\ell_2+1}$ from
   =Y_filter=, $\sqrt{2L+1}$ from =L_factor=, leaving the natural
   =pixell=/=healpy= normalization.
2. *±spin pairing.* Fuse recipes whose $(\mathrm{spin}_X, \mathrm{spin}_Y,
   \mathrm{spin}_L)$ flip together and share the same filters into a
   single complex SHT call (exploiting the fact that
   =alm2map_spin(spin_transform=s)= already computes both $\pm s$
   components).
3. *pixell backend.* Emit a numpy callable =compiled(X_alm, Y_alm) →
   (phi_alm, curl_alm)= that runs =cs.alm2map_spin=,
   pixel-multiplies, =cs.map2alm_spin=, and accumulates.
4. *Bit-for-bit validation.* For TT, compare against
   =falafel.qe.qe_temperature_only= on synthetic alms. For polarization
   estimators (EE, EB, BB, TB), compare against =qe_pol_only= and
   =qe_spin_pol_deflection= — these need the complex $(E+iB, E-iB)$
   spin-±2 pair assembly, deferred to milestone 2.
