...
=EstimatorTerm= lists — not only the lensing templates for which
falafel has hand-coded primitives.  The same compilation path
(=compile_estimator → strip_gamma → compile_*_native=) applies to
estimators for CMB rotation, patchy τ, point sources, or any new
weight function =W= a user writes symbolically.  The falafel-delegating
backend A stays as a regression oracle for the 6 lensing QEs.

TE/TB/EB haven't been wired into the native backend yet but are
mechanical extensions of the existing EE/BB pattern.
