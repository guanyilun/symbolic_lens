...
HEALPix NSIDE=256, LMAX=200; see =archive/probes/probe_regression_final.py=):
...
=scripts/demos/demo_search_fisher.py= is kept as an illustration of the pure-Fisher
pathology.
...
The remaining pieces for a production search tool (grammar enumerator,
e-graph collapse, gauge-fixing, discovery objective) are listed under
[[Future Work]] below.
...
- =scripts/demos/demo_search_fisher.py= — pure-Fisher ranking showing the gauge
  pathology.
