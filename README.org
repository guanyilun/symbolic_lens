...
** Estimator compiler milestone 2 — polarization (next)

Extend the backend to handle spin-2 inputs (E, B) via falafel's
=qe_spin_pol_deflection=. Four more estimators to cover: TE, TB, EE,
EB, BB. Each requires identifying the appropriate pair structure in the
symbolic recipe and mapping to the corresponding falafel primitive.
The ζ⁻ = i factor on the W⁻ weights is carried symbolically as
explicit =I= in the coefficient and will split cleanly into E/B legs
of the ($E+iB$, $E-iB$) spin-±2 pair.
