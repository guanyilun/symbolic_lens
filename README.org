...
** Estimator compiler milestone 3 — native SHT backend ✅

=sym_utils/estimator_native.py= implements the same pipeline without a
...
compilation path (=compile_estimator → strip_gamma → compile_*_native=)
applies to estimators for CMB rotation (anisotropic birefringence, α),
patchy τ, point sources, or any new weight function =W= a user writes
symbolically.

*Full 6-estimator parity (TT/EE/BB/TB/EB/TE)* — all match backend A
bit-for-bit at ~10⁻¹⁷ machine precision; see
=test_estimator_native_vs_backendA.py=.

*Demo: new physics beyond lensing.*  =demo_rotation_estimator.py= builds
...
*End-to-end rotation pipeline.*  =compile_rot_eb_native= in
...
=test_rotation_native.py= runs the pipeline on synthetic EE/BB alms
and confirms that the output is pure-real with a plausible
reconstruction-noise-like spectrum.

** Estimator compiler milestone 4 — generic emitter ✅

Status: =compile_native= in =sym_utils/estimator_native.py= now executes
...
=compile_*_native= bit-for-bit (std/mean ≲ 5e-15); EB retains a 5.6%
Hu-Okamoto structural difference (documented below).

*Why a generic emitter?*  Milestones 1-3 hand-coded one emitter per
estimator (=compile_tt_native=, =compile_ee_native=, …).  Each one
...
=probe_int_trunc_error_summary.py=):
...
*Final regression* (=compile_native= vs hand-coded =compile_*_native=,
HEALPix NSIDE=256, LMAX=200; see =probe_regression_final.py=):
...
The constant 0.14105 offset on TT/TE/TB/EE/BB is the per-estimator
=pair_coeff= that hand-coded emitters absorb internally.  The generic
emitter intentionally does not bake this in — the user multiplies it
externally, since for new-physics estimators (rotation, source, …) the
right normalization depends on the physics, not on the SHT recipe.

*Why EB is different.*  =compile_eb_native= implements the
...
=compile_estimator → strip_gamma → fuse_spin_pairs → compile_native=
...
*Probe scripts (in repo root, retained for diagnostics).*

- =probe_regression_final.py= — canonical regression across all 6
  lensing estimators after milestone 4.
- =probe_int_trunc.py= — monkey-patched =compile_native= demonstrating
  the int-truncation rule alone brings EE/BB/TB/TE from std/mean
  ~5e-3 down to ~1e-15.
- =probe_int_trunc_error_summary.py= — quantifies the upstream falafel
  int-truncation error per-ℓ and signal-weighted up to ℓmax = 3000;
  the source of the impact tables documented above.
- =probe_qe_shear_final.py= — canonical qe_shear regression:
...
- =probe_te_halves.py= — TE pol-half vs temp-half breakdown that led
  to rule 5.
- =probe_eb_per_half.py= — EB structural breakdown showing the
  Hu-Okamoto convention difference.
- =probe_all_plans_inspect.py=, =probe_ee_plan_inspect.py=,
  =probe_eb_plan_inspect.py=, =probe_tb_plans.py= — plan
  coefficient/signature dumps for each estimator.

** Estimator compiler milestone 5 — unified compile + search prototype ✅

Status: =sym_utils/compile_qe.py= compiles BOTH the estimator and the
...
=norm_lens.qtt= (max rel diff $1.3 \times 10^{-14}$ over
$L \in [20, 200]$).  A small search-tool prototype
(=demo_search_unified.py=) shows the correct Hu-Okamoto weight rises
...
estimator, A_L_fn, N_L_phi_fn = compile_qe(
    g_symbolic,            # sympy expression for g = f / (Δ · hCxx · hCyy)
    lmax=200,
    Delta=2,
    hCxx=hCT, hCyy=hCT,    # IV-filter Function classes in g
    user_funcs=[hCT, CT],  # all Function classes in g (positional order
                           # of the returned fn's spectrum args)
...
*A_L derivation in one line.*  For $g = f / (\Delta \hat C_{\rm XX}
\hat C_{\rm YY})$:
...
(The second form avoids the user writing $f$ separately; compile_qe
just squares the user's $g$, multiplies by $\Delta \hat C \hat C$,
=cancel()= s the $1/\hat C$ factors, and hands the result to
=L12SumCompiler=.  For ζ^- estimators like TB/EB the imaginary
coefficients are carried through the atomic tree correctly because
the normalization integrand is $|g|^2 = g \cdot \bar g$; in practice
$g \cdot g$ gives the same result since the $i$ factors square to
$-1$ and the integrand contains a consistent sign.)

*Search prototype (=demo_search_unified.py=).*  Enumerate 5 candidate
...
*Gauge subtlety with pure-Fisher scoring.*  A naive FOM
=Fisher = sum (2L+1) / N_L^phi= appears attractive (no oracle
$\phi_{\rm true}$ needed) but is *gauge-dependent* under $g \to
\alpha g$: because the user-supplied $g$ treats $f$ as derived
($f = g \cdot \Delta \hat C \hat C$), rescaling $g$ rescales both
response and noise non-uniformly.  Pure-Fisher therefore ranks
strawman candidates above the correct f_TT in the demo.  The
gauge-invariant quantity is the cross-correlation coefficient of the
A_L-normalized output with any proxy $\phi$ signal (simulation,
galaxy-cross-correlation, etc.) — that's what the prototype uses.
=demo_search_fisher.py= is kept as an illustration of the pure-Fisher
pathology.

*What milestone 5 unlocks.*  A symbolic-search pipeline where each
candidate weight is compiled into a *self-consistent triple*
(estimator, A_L, N_L^phi) in one call, evaluated on data, and ranked.
Remaining pieces for a production search tool:

1. *Grammar enumerator* — currently candidates are hand-picked.  BFS
   over the expression grammar $\{\text{wigner\_3j}, a(\ell, s),
   C_{\rm spectrum}, P\}$ up to a size budget (~50 lines).
2. *E-graph collapse* — many enumerated candidates differ only by
   scalar multiples, 3j column permutations, or $P$ absorption.
   Equality saturation (already flagged under [[Future Work]] below)
   would canonicalize them automatically.
3. *Gauge-fixing for Fisher ranking* — if oracle-free scoring is
   required, rescale each candidate's $g$ so that $A_L$ at a
   reference $L$ matches a common value, then compare $N_L^\phi$
   elsewhere.  One line in the scoring pass.
4. *Objective for unknown-field discovery* — the deepest gap, and
   outside the engine scope.  Candidates: max non-Gaussianity
   response, orthogonality to known contaminants, cross-correlation
   with an external tracer.

Items 1–3 are ~a week each.  Item 4 is the actual research question.

*Scripts retained for diagnostic purposes.*

- =demo_search_unified.py= — canonical search demo with both
  cross-correlation and Fisher rankings.
- =demo_search_fisher.py= — pure-Fisher ranking showing the gauge
  pathology.
- =demo_search_diag.py= — per-L breakdown that surfaced the need for
  (2L+1)-weighted $\langle r(L) \rangle$ (naive global xcorr is
  dominated by low-$L$ $\phi$ power where the QE has no support).
- =demo_search_lensing.py= — earlier iteration using cross-
