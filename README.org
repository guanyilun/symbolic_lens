...
- =scripts/demos/demo_search_enum.py= — grammar enumeration over the DSL,
  gauge-fixed Fisher ranking, references injected for comparison
  (see [[Milestone 6]]).
- =scripts/demos/demo_search_oracle.py= — two-pass Fisher+oracle
  search: enumerate → Fisher prune → cross-correlate with simulated
  phi_true → rerank.
- =scripts/demos/demo_search_harness.py= — same lensing-TT case run
  through the generic =sq.run_search= harness, with multi-seed
  averaging support (see [[Milestone 7]]).
- =scripts/demos/demo_search_rotation.py= — rotation-EB case (second
  case landed — see [[Milestone 7]]).  Validated that the pipeline
  handles W^- / imaginary-coefficient weights after a targeted
  ``g·conj(f)`` fix in compile_qe_from_f.

** Milestone 6 — grammar enumeration + two-pass search ✅

Status: the pieces from milestone 5's prototype (=compile_qe_from_f=,
=fisher_fom=) are wired to a BFS enumerator over the compiler's DSL,
and a two-pass ranker (Fisher prune → oracle cross-correlation) brings
Hu-Okamoto $f^{TT}$ from top 18% under Fisher alone to top 1% under
the combined score.

*Enumerator* (=symqe.engine.enumerate=).  BFS over

$$\text{coeff} \cdot \gamma_f \cdot X(\ell_1) \cdot Y(\ell_2) \cdot
  \text{w3j}(\ell, \ell_1, \ell_2;\, m_L, m_X, m_Y) \cdot
  \text{parity}$$

with ``parity_factors`` defaulting to $(1, P, 1+P, 1-P)$ — the latter
two are Namikawa's $q_\pm = (1 \pm P)/2$ selectors (up to scale),
which is how $W^{x,\pm}$ parity gets into a single candidate expression.
``symmetrize_l1l2`` optionally emits $\text{expr} + \text{swap}(\text{expr})$,
which is how TT/EE/BB-style symmetric estimators are built in one step.

Plain structural dedup via =canonical_key= — the canonical form is
the sorted tuple of =EstimatorTerm= signatures from
=compile_estimator=, so 3j column permutations and $P^2 = 1$ are
already collapsed by the engine.

*Two-pass search* (=scripts/demos/demo_search_oracle.py=).  First pass:
score all candidates by gauge-fixed Fisher FOM (cheap, no simulation);
keep the top-K.  Second pass: simulate a lensed CMB with known
$\phi_{\rm true}$, apply each surviving estimator, form
$\hat\phi = A_L \cdot \hat\psi$, rank by
=cross_correlation_score= — the mode-count-weighted
$\langle r(L) \rangle$ between $\hat\phi$ and $\phi_{\rm true}$.

*Regression on lensing TT* (lmax=100, nside=64, 4050 enumerated):

| Reference candidate | Fisher rank | Oracle rank | Oracle $\langle r \rangle$ |
|---------------------+-------------+-------------+---------------------------|
| $f^{TT}$ correct    | 729 / 4053  |   8 / 1001  | +0.153 |
| $f^{TT}$ wrong ladder | 217 / 4053 |   6 / 1001  | +0.159 |
| $f^{TT}$ wrong m-row | 4053 / 4053 | 1001 / 1001 | $-\infty$ |

Oracle discriminates correctly (wrong-m-row eliminated; correct enters
top 10).  Wrong-ladder edges out correct by ~4% — within single-
realization noise at lmax=100; averaging over sims or going to
lmax>200 should close the gap.  Top-5 oracle ranks are enumerated
candidates *better* than any hand-coded reference: close structural
siblings of Hu-Okamoto with ladder on one leg only, $\langle r \rangle
\approx 0.168$.  That's the enumerator doing its job — surfacing
"nearby variants" the user didn't hand-pick.

*Runtime budget.*  ~12 min end-to-end (enumerate 32s, Fisher 368s,
oracle rerank on top 1000: 449s).  Bottleneck is =NormCompiler=
being invoked twice per candidate (for $A_L^{-1}$ and for the noise
variance); cache-sharing is an obvious optimization.

*What milestone 6 unlocks.*  The pipeline is now a bona-fide symbolic
search tool: the user writes down the grammar scope and the physical
target (via simulation or cross-tracer), and the pipeline explores,
scores, and ranks candidate weight expressions without hand-picking.
The remaining Future Work item is the *objective for unknown-field
discovery* — what FOM replaces ``phi_true cross-correlation`` when the
target physics is genuinely unknown.

** Milestone 7 — generic multi-case harness ✅

Status: =symqe.engine.search= wraps the milestone-6 pipeline in a
reusable entry point so that each estimator case (lensing TT,
rotation EB, source TT, …) is a configuration rather than a
hand-written script.

*API.*  A case is one ``SearchCase`` dataclass instance:

#+begin_src python
case = sq.SearchCase(
    name="lensing-TT",
    lmax=100,
    grammar=dict(m_max=2, coeffs=(1,), parity_factors=(1, 1+P),
                 max_leg_factors=2, ladder_spins=(0, 2, -2),
                 spectra_X={"CT": CT}, spectra_Y={"CT": CT},
                 restrict_m_L=(-1, 1), symmetrize_l1l2=True),
    simulate=lambda seed: (X_input, Y_input, phi_true),  # user-supplied
    compile_kwargs=dict(Delta=2, hCxx=hCT, hCyy=hCT,
    ...
    output_key=+1, top_k_for_oracle=1000,
    references=[("f_TT correct", f_TT_correct()), ...],
)
result = sq.run_search(case, seeds=(0, 1, 2, 3))
sq.print_summary(result, n_show=15)
#+end_src

Enumeration and Fisher scoring run once per case (spectra are
seed-independent); the oracle pass loops over seeds.  Per-candidate
oracle scores are aggregated into (mean, std) across seeds and the
ranking is stable on the mean.

*Performance bits enabled in this milestone.*
- =score_candidates(..., return_bundles=True)= keeps the compiled
  (estimator, $A_L$, $N_L^\phi$) bundle so the oracle pass skips
  recompilation.
- Shared GLQuad cache in =l12_sum._cached_glq= keyed on
  $(\ell_{\max}, r\ell_{\max})$ — small win, but removes redundant
  work across candidates in a batch.
- =sq.oracle_rerank= is a library-level function; the harness and
  ad-hoc scripts share one implementation.

*What this unlocks for paper work.*  Adding a new case is now one
``SearchCase`` dict — you supply a ``simulate`` callable that returns
(X_input, Y_input, oracle_alm) for a given seed, and a grammar
restriction.  The infrastructure for multi-seed statistical averaging
is in place (``aggregate_across_seeds``); exercising it at scale
is the next step.

Regression under the new harness reproduces the milestone-6 ranks
bit-for-bit: f_TT correct at 8/1001, wrong ladder 6/1001, wrong
m-row 1001/1001 — pure refactor.

*Second case landed: rotation-EB.*  =scripts/demos/demo_search_rotation.py=
configures a birefringence search (primordial $C_{BB} = 0$, random
$\alpha(\hat n)$ rotating Q ± iU in pixel space, E/B observed).
Tight grammar: 939 candidates with spectra $\{C_E, C_B\}$, no
ladders, $\{\pm 1, \pm i\}$ coefficients, parity factors
$(1, P, 1+P, 1-P)$, $m_L = 0$.  Results (lmax=100, nside=64, 1 seed):

| Reference                 | Oracle rank | ⟨r(L)⟩  |
|---------------------------+-------------+---------|
| $f^{\alpha, EB}$ Namikawa |   4 / 939   | +0.40   |
| Namikawa half ($W \cdot C_E$ swap) | 142 / 939 | −0.40 |
| Namikawa half ($W \cdot C_B$)      | 939 / 939 | $-\infty$ |

Correct weight at top 0.4%.  The $W \cdot C_E$ half lands with
*negative* cross-correlation because the full Namikawa weight is an
antisymmetric sum $W \cdot C_B - W^{\rm swap} \cdot C_E$ — the
single-atom half has the opposite sign.  The $W \cdot C_B$ half
correctly collapses to ``−∞`` ($C_{BB} = 0$ primordial means no
response).

*Latent bug surfaced during rotation work.*  ``compile_qe_from_f``
originally built the $A_L^{-1}$ integrand as $g \cdot f$, which for
real-coefficient weights equals $g^2 \cdot \Delta \cdot \hat C \hat C$
(correct) but for complex-coefficient weights (W^-, rotation, any
ζ^- = i) yields a NEGATIVE integrand that the ``np.where(inv>0, ...)``
branch silently mapped to ``inf``.  Every W^- reference scored
``-∞`` on Fisher before the fix.  Now uses $g \cdot \bar f$ via a
targeted ``I → -I`` substitution (sympy's ``conjugate()`` leaves an
opaque wrapper the atomic-tree bridge can't lower).  Transparent
for real-f cases (TT references keep Fisher FOMs bit-identical).
...
Milestones 5 and 6 established the compilation triple (estimator,
A_L, N_L^phi), gauge-fixed Fisher, and the grammar-enumerator
two-pass search.  What remains open:

1. *E-graph collapse* — plain structural dedup (from
   =canonical_key=) already collapses 3j column permutations and
   $P^2 = 1$.  Still-redundant candidates (e.g., coeff-sign
   variants that produce identical Fisher/oracle scores) can be
   merged via equality saturation once search sizes force the
   issue.  Currently manageable (~4000 candidates for a TT
   grammar).
2. *Objective for unknown-field discovery* — the deepest gap, and
   outside the engine scope.  When the target physics is genuinely
   unknown there is no oracle $\phi_{\rm true}$ to cross-correlate
   against.  Candidates: max non-Gaussianity response,
   orthogonality to known contaminants, cross-correlation with an
   external tracer.  This is the actual research question.
...
[[Milestone 5]].  The grammar enumerator was also on this list, now
done under [[Milestone 6]].)

*Search-space scope.*  The compiler's de facto DSL for an estimator
term is

$$\text{coeff} \cdot L(\ell) \cdot X(\ell_1) \cdot Y(\ell_2) \cdot
  \text{w3j}(\ell, \ell_1, \ell_2;\, m_L, m_X, m_Y) \cdot P^{\,0\,|\,1}$$

with $m_L + m_X + m_Y = 0$, integer $|m_*| \le 3$, and legs $L/X/Y$
each a product of atoms in one $\ell$-variable drawn from
$\{\sqrt{\text{linear-in-}\ell}, a(\ell, s), C_\ell^*, (2\ell+1)^k,
\gamma_f\}$.  Coefficients in $\mathbb{Z} + i\mathbb{Z}$ (the
imaginary part comes from $\zeta^- = i$ on W^-).  P marks 3j
column-swap parity and reduces under $P^2 = 1$.

*This is broader than Namikawa.*  Namikawa picks a few specific
m-triples (e.g. (1,0,-1), (1,2,-3), (-1,2,-1)) and specific ladder
products.  Any m-triple with $|m| \le 3$ compiles, any user-declared
spectrum function compiles, chained ladders compile (qe_shear uses a
4-factor chain, validated), non-optimal filters compile.  Physics
claim: if an unknown field modulates a CMB 2-point function linearly,
its quadratic estimator weight lives in this DSL — the spin-coupling
algebra forces Wigner-3j with spins in the m-row.

*Outside the DSL:* multi-3j per term (that's the W² normalization
side — goes through =compile_norm= / =NormCompiler=), non-factorable
legs (e.g. $C(\ell_1 + \ell_2)$), $|\text{spin}| > 3$ (SHT primitive
cap), trispectrum / 4-point estimators.

*Enumerator defaults* (from =symqe.enumerate_candidates= —
the TT demo uses the tight config =coeffs=(1,)=, ladders
$\{0, \pm 2\}$, parity $\{1, 1+P\}$, $|m| \le 2$, CT spectrum, 4050
candidates).  User-facing knobs: ``m_max``, ``coeffs``,
``parity_factors``, ``max_leg_factors``, ``ladder_spins``,
``spectra_X``/``spectra_Y``, ``restrict_m_L``, ``symmetrize_l1l2``.
Default parity factors $(1, P, 1+P, 1-P)$ cover $q_\pm$ selection.

Deferred questions (still open at 2026-04-23):
- Whether to auto-restrict spectrum atoms to the input-field type
  (T-leg gets only $C^T$-like spectra) vs allow mismatches that
  score badly but might surface unexpected couplings.  Currently
  ``permissive``; the ``field_gating`` kwarg is reserved.
- Whether to admit a user-declared "unknown spectrum" symbol for
  new-physics discovery (needs a runtime value — Monte Carlo prior,
  parameterized fit, or cross-tracer proxy).

See also =memory/search_space.md= and =memory/enumerator_state.md=
for the full scope analysis and current search-pipeline state.
